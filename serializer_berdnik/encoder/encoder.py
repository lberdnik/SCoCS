...
from .complement import is_iterable, get_class_that_defined_method
...
class Encoder:
    ...
    @classmethod
    def _encode_property(cls, prop: property):
        data = dict(fget=cls.encode(prop.fget), fset=cls.encode(prop.fset), fdel=cls.encode(prop.fdel))
        return cls._mark_data(data, TYPE.PROPERTY)

    @classmethod
    def _decode_property(cls, encoded):
        data = cls.decode(cls._get_data(encoded))
