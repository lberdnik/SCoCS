...
class XmlSerializer(BaseSerializer):
    ...
    @classmethod
    def _dumps(cls, encoded):
        if isinstance(encoded, bool):
            return f"<bool>{str(encoded)}</bool>"

        if isinstance(encoded, int):
            return f"<int>{str(encoded)}</int>"

        if isinstance(encoded, float):
            return f"<float>{str(encoded)}</float>"

        if isinstance(encoded, str):
            return f"<str>{encoded}</str>"

        if isinstance(encoded, type(None)):
            return f"<none>None</none>"

        if isinstance(encoded, (list, tuple)):
            return f"<list>{''.join(list(map(cls._dumps, encoded)))}</list>"

        if isinstance(encoded, dict):
            data = "".join(
                [f"<{key}>{cls._dumps(value)}</{key}>" for (key, value) in encoded.items()]
            )
